...
<body>
    ...
    <div class="register-container">
        ...
        <form method="post">
            <input type="text" name="nomu" placeholder="Full Name" required>
            <input type="email" name="emailu" placeholder="Email Address" required>
            <input type="tel" name="telu" placeholder="Phone Number" required>
        ...
        if(isset($_POST['btn'])){
            ...
            $rech=mysqli_query($conn,"select * from utilisateur
            where emailu='$emailu'");
            if(mysqli_num_rows($rech)==1){ 
                echo '<div class="error-message">⚠️ This email already exists!</div>';
            }
            else{
                $ajouter=mysqli_query($conn,"insert into 
                utilisateur(nomu,emailu,telu,mdpu)
                values('$nomu','$emailu','$telu','$mdpu')");
                echo '<div class="success-message">✅ Success! You can now login.</div>';
            }
        }
        ?>
    </div>
